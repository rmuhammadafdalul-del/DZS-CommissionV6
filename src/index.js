...
if (!need("ORDER_CATEGORY_ID") && !need("TICKET_CATEGORY_ID")) process.exit(1);
if (!need("TESTIMONI_CHANNEL_ID") && !need("FEEDBACK_CHANNEL_ID")) process.exit(1);
if (!need("STAFF_ROLE_ID")) process.exit(1);

const ORDER_CATEGORY_ID = process.env.ORDER_CATEGORY_ID?.trim() || process.env.TICKET_CATEGORY_ID?.trim();
const TESTIMONI_CHANNEL_ID = process.env.TESTIMONI_CHANNEL_ID?.trim() || process.env.FEEDBACK_CHANNEL_ID?.trim();
...
function skinSizeSelectMenu() {
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .setPlaceholder("🧑‍🎨 Pilih ukuran skin...")
      .addOptions(
        { label: "64×64", description: "Minecraft Skin 64×64", value: "64", emoji: "🟨" },
        { label: "128×128", description: "Minecraft Skin 128×128", value: "128", emoji: "🟨" },
        { label: "256×256", description: "Minecraft Skin 256×256", value: "256", emoji: "🟨" },
        { label: "512×512", description: "Minecraft Skin 512×512", value: "512", emoji: "🟨" }
      )
  );
}
...
function animationStyleSelectMenu() {
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .addOptions(
        {
          label: "Basic Style — 2k/Sec",
          description: "Max 30 detik • Basic Lighting",
          value: "basic",
          emoji: "🟢"
        },
        {
          label: "Medium Style — 7k/Sec",
          description: "Durasi 1 menit • Max 4 Char",
          value: "medium",
          emoji: "🟡"
        },
        {
          label: "Hight Style — 10k/Sec",
          description: "Skin 256x/512x • Full Lighting",
          value: "hight",
          emoji: "🔴"
        }
      )
...
async function createTicket(interaction, category, size) {
  ...
  // Semua ticket/order selalu dibuat di bawah Category ORDER.
  const categoryChannel = guild.channels.cache.get(ORDER_CATEGORY_ID);
  if (!categoryChannel || categoryChannel.type !== ChannelType.GuildCategory) {
    return interaction.reply({
      content: "❌ ORDER_CATEGORY_ID salah atau bukan Category Channel.",
      ephemeral: true
    });
  }
  ...
  try {
    ...
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.commandName === "setup-feedback") {
        ...
        return interaction.reply({
          content: `✅ Panel feedback dipasang di <#${feedbackChannel.id}>.\n📌 Ticket/order tetap dibuat di Category ORDER: <#${ORDER_CATEGORY_ID}>.`,
          ephemeral: true
        });
      }
